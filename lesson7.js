...
function startGame() {
    ...
    // setInterval(createRock, 5000);  //каждые 5000мс создаем препятствия (ЛОВУШКА)
}
...
// СОЗДАЕМ ПРЕПЯТСТВИЕ НА ПОЛЕ - ЛОВУШКА, КОТОРАЯ БУДЕТ УМЕНЬШАТЬ РЕЗУЛЬТАТ.
...
function createRock() {       // Создаем препятствия
...
function finishTheGame_win() {
    gameIsRunning = false;
    clearInterval(snake_timer);
    alert('Ура, Вы победили! Ваш результат: ' + score.toString());
    score=0;
}
